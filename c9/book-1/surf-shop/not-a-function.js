...
const convertDataForAccounting = () => {
    for (const item of inventory) {
        const allUpperCase = {
            upperDescription: item.description.toUpperCase(),
            upperPrice: item.price,
            upperType: item.type.toUpperCase()
        }
        console.log(`${allUpperCase.upperDescription} - ${allUpperCase.upperType} - ${allUpperCase.upperPrice}`)
    }
}
